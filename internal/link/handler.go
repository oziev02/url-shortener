...
func (handler *LinkHandler) Create() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		link := NewLink(body.Url) // создаем обЪект Link
		for {
			existedLink, _ := handler.LinkRepository.GetByHash(link.Hash) // проверяем существует ли такая ссылка
			if existedLink == nil {
				break // если такой ссылки не существует, то выходим из цикла
			}
			link.GenerateHash() // если такая ссылка существует, то заново генерируем хэш
		}
